...
func writeComponentFile(filename string, classes []*CSSClass, component string, config Config) error {
	...
	componentTitle := strings.Title(strings.ReplaceAll(component, "_", " "))
...
// sortClasses sorts by layer order, then alphabetically
func sortClasses(classes []*CSSClass) []*CSSClass {
	// Make a copy to avoid modifying the input
	sorted := make([]*CSSClass, len(classes))
	copy(sorted, classes)

	// Sort by layer order, then alphabetically by GoName
	sort.Slice(sorted, func(i, j int) bool {
		// Layer priority: reset < base < tokens < components < utilities
		...
		iOrder := layerOrder[sorted[i].Layer]
		jOrder := layerOrder[sorted[j].Layer]
